...
description: Implements a story by following its `plan` sticky comment precisely, recording anything surprising or missing as the `learnings` sticky comment. Dispatched by `/factory-implement` during the implementation phase.
model: sonnet
---
...
- DO NOT dispatch any review or toolchain sub-agent (`factory-review`, `factory-review-csharp`, `factory-review-react`, `factory-review-spec`, `factory-fixer`) or invoke `/code-review`. Review is a separate later phase run by the orchestrator — it is not your job. When the plan says to "verify" a piece of work, run the concrete command yourself (e.g. `dotnet build`, the test command, the lint command) directly via Bash; never delegate verification to a reviewer agent.
