...
- **Components in play** — which existing components/projects/modules the work touches; whether any *new* component is introduced; whether responsibilities move between existing components. Establish the full set of nodes (and which are new / modified / untouched-but-relevant) before pinning down the edges.
...
```mermaid
graph TD
  %% legend: new / modified / untouched conveyed via classDef styling below
  A[Existing Component]:::untouched --> B[Changed Component]:::modified
  B --> C[New Component]:::new
  classDef new fill:#d4edda,stroke:#28a745;
  classDef modified fill:#fff3cd,stroke:#ffc107;
  classDef untouched fill:#e2e3e5,stroke:#6c757d;
```
